...
Auto-provisions a project, installs the package, patches `convex/convex.config.ts`, creates `convex/analytics.ts` with the write key inlined, inserts the browser script tag, and installs a skill file for AI agents. No write key required, one is created for you. Pass an existing key to reuse a project:
